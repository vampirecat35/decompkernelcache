...
#ifndef CF_OPEN_SOURCE
#if (TARGET_OS_MAC && !(TARGET_OS_EMBEDDED || TARGET_OS_IPHONE)) || (TARGET_OS_EMBEDDED || TARGET_OS_IPHONE)
CF_EXPORT void CFNotificationCenterSetSuspended(CFNotificationCenterRef nc, Boolean flag);
#endif
#endif // !CF_OPEN_SOURCE
...
CF_EXPORT
CFPropertyListRef _CFURLCopyExtendedPropertyListPrimitive(CFURLRef url, AliasPtr alias);

CF_EXPORT
CFPropertyListRef _CFURLCopyExtendedPropertyListRepresentation(CFURLRef url);

CF_EXPORT
CFDataRef _CFURLCopyAliasFromPropertyListRepresentation(CFPropertyListRef pListRepresentation);
#endif
#endif // !CF_OPEN_SOURCE
...
#ifndef CF_OPEN_SOURCE
    CFSystemVersionBarolo = 7,		/* 10.7 alias */
#endif
    CFSystemVersionLion = 7,		/* 10.7 */
    CFSystemVersionMountainLion = 8,    /* 10.8 */
#ifndef CF_OPEN_SOURCE
    CFSystemVersionZin = 8,		/* 10.8 alias */
    CFSystemVersionCab = 9,		/* 10.9 alias */
#endif
    CFSystemVersionMax,                 /* This should bump up when new entries are added */

#ifndef CF_OPEN_SOURCE
    CFSystemVersionEmbeddedMin = 1000,
    CFSystemVersionTimberline =  1000,	/* 2.2 */
    CFSystemVersionKirkwood = 1001,     /* 3.0 */
    CFSystemVersionNorthstar = 1002,    /* 3.1 */
    CFSystemVersionWildcat = 1003,	/* 3.2 */
    CFSystemVersionApex = 1005,		/* 4.0 */
    CFSystemVersionBaker = 1006,	/* 4.1 */
    CFSystemVersionJasper = 1007,	/* 4.2 */
    CFSystemVersionDurango = 1008,	/* 4.3 */
    CFSystemVersionTelluride = 1010,	/* 5.0 */
    CFSystemVersionHoodoo = 1012,	
    CFSystemVersionSundance = 1015,
    CFSystemVersionInnsbruck = 1020,
    CFSystemVersionEmbeddedMax          /* This should bump up when new entries are added */
#endif
};
...
#ifndef CF_OPEN_SOURCE
#if (TARGET_OS_MAC && !(TARGET_OS_EMBEDDED || TARGET_OS_IPHONE))
/* kCFCompare flags for AppleScript support. DO NOT USE THEM IF YOU'RE NOT APPLESCRIPT. Contact me (aki@apple.com) if you need them.
 */
enum {
    kCFCompareIgnorePunctuation = (1UL << 28), // Ignores characters in kCFCharacterSetPunctuation
    kCFCompareIgnoreHyphens = (1UL << 29), // Ignores characters in the Unicode General Category Pd
    kCFCompareIgnoreWhitespaceAndNewline = (1UL << 30), // Ignores characters in kCFCharacterSetWhitespaceAndNewline
};

/* Comparison SPI tailored for HFS filenames. This SPI is for Finder's use only. DO NOT USE THEM IF YOU'RE NOT Finder. Contact me (aki@apple.com) if you need them.
 */
/* Recognized flags are kCFCompareCaseInsensitive, kCFCompareNonliteral, and kCFCompareForcedOrdering only. Since the logic is tailored for HFS filename comparison, the result might no match CFStringCompareWithOptionsAndLocale
 */
CF_EXPORT
CFComparisonResult _CFStringCompareForHFS(CFStringRef theString1, CFStringRef theString2, CFStringCompareFlags compareOptions);
#endif
#endif /* !CF_OPEN_SOURCE */
...
#ifndef CF_OPEN_SOURCE
/* Enhanced formatting config keys
 */
CF_EXPORT CFStringRef kCFStringFormatSpecTypeKey;
CF_EXPORT CFStringRef kCFStringPluralRuleType;
CF_EXPORT CFStringRef kCFStringGenderRuleType;

CF_EXPORT CFStringRef kCFStringFormatValueTypeKey;
CF_EXPORT CFStringRef kCFStringFormatLocaleKey;

CF_EXPORT CFStringRef kCFStringLocalizedFormatKey;
 ...
 */
CF_EXPORT bool CFAttributedStringGetBidiLevels(CFAttributedStringRef attributedString, CFRange range, int8_t baseDirection, uint8_t *bidiLevels);
