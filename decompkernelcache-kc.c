...
#if (__MAC_OS_X_VERSION_MIN_REQUIRED >= 1060) || (__IPHONE_OS_VERSION_MIN_REQUIRED >= 40000) || (__WATCH_OS_VERSION_MIN_REQUIRED >= 20000) || (VISIONOS_VERSION_MIN_REQUIRED >= 10000) || defined(__linux__) || defined(_WIN32) || defined(WIN32)
...
uint8_t saveKexts(unsigned char *aFileBuffer, const char *dir)
{
    ...
    if (1/*(prelinkTextSegment64) && (prelinkInfoSegment64) && (is32bit == 0)*/)
    {
		    ...
		    outdir = "kexts";
                ...
                for (i = 0; i < kextCount; i++)
                {
                    ...
                    if (executableName)
                    {
                        ...
                        if ( strcmp(kextIdentifierBuffer, "ALF") == 0) base=1;
                        
                        if (kextSourceAddress != NULL)
                        {
                            CFNumberGetValue(kextSourceAddress, kCFNumberSInt64Type, &sourceAddress);
                            if (base) sourceAddress+=0x100000;
                            if (sourceAddress==0x80000000000fffff) continue;
                       ...
                       // codeSignature = (struct linkedit_data_command *)find_load_command(machHeader, LC_CODE_SIGNATURE);
                        ...
                        struct mach_header_64 *mhp=(struct mach_header_64 *)((unsigned char *)(uintptr_t)aFileBuffer + offset);

                        struct segment_command_64 *sgp=0;
                        struct symtab_command *sp=0;
                        struct linkedit_data_command *sp2=0;
                        struct dysymtab_command *sp3=0;
                        struct section_64 *spp;
                        
                        //mhp->flags=0x85;
                        
                        struct dysymtab_command* dynSymbolTable = NULL;
                        ...
                        for(i = 0; i < mhp->ncmds; i++){
                            if(sgp->cmd == LC_SEGMENT_64)
                            {
                                if (strcmp(sgp->segname,SEG_LINKEDIT) == 0){
                                    if (base) sgp->vmaddr &= ~0xffffff8000000000;
                                    unslidLinkEditBase = (uint8_t*)(sgp->vmaddr - sgp->fileoff);
                                    offsetlb=sgp->fileoff;
                                    sgp->vmaddr=offset1m;
                                    sgp->fileoff=offset1;
                                    offsetl=sgp->fileoff;
                                    offsetlm=sgp->vmaddr;
                                    offsetls=sgp->filesize;
                                }
                                else {
                                    ...
                                    if ( strcmp(sgp->segname, "__TEXT") == 0 ) {
                                        ...
                                        spp = (struct section_64 *)((char *)sgp +
                                                     sizeof(struct segment_command_64));
                                        
                                        for(j = 0; j < sgp->nsects; j++){
                                           
                                            if ( strcmp(spp->sectname, "__got") == 0 || strcmp(spp->sectname, "__mod_init_func") == 0 || strcmp(spp->sectname, "__mod_term_func") == 0 || strcmp(spp->sectname, "__bss") == 0 || strcmp(spp->sectname, "__kalloc_type") == 0 || strcmp(spp->sectname, "__kalloc_var") == 0) {
                                                if (base) sgp->vmaddr &= ~0xffffff8000000000;
                                                if (base) spp->addr &= ~0xffffff8000000000;
                                            struct section_64* sectionsStart = (struct section_64*)((char*)sgp + sizeof(struct segment_command_64));;
                                                struct section_64* sectionsEnd = &sectionsStart[sgp->nsects];
                                                for (struct section_64* sect=sectionsStart; sect < sectionsEnd; ++sect) {
                                                    if (base) sect->addr &= ~0xffffff8000000000;
                                                        const uint32_t pointerCount = (uint32_t)(sect->size / sizeof(uintptr_t));
                                                        uintptr_t* const symbolPointers = (uintptr_t*)(sect->addr + slide);
                                                        for (uint32_t j=0; j < pointerCount; ++j) {
                                                            if (base) symbolPointers[j] &= ~0xffffff8000000000;
                                                            
                                                            symbolPointers[j] &= ~0xFFFF000040000000;
                                                            
                                                            if ( strcmp(spp->sectname, "__got") != 0)
                                                            if (symbolPointers[j] >= offsetr) symbolPointers[j] -= offsetr;
                                    ...
                                    if (!offsetr) offsetr=sgp->fileoff;
                                    if (!offsetrm) offsetrm=sgp->vmaddr;
                                    offset1=sgp->fileoff+sgp->filesize-offsetr;
                                    //offset1m=sgp->vmaddr+sgp->vmsize-offsetrm;
                                    
                                    if (sgp->vmaddr>=offsetrm)  sgp->vmaddr-=offsetrm;
                                    if (sgp->fileoff>=offsetr)  sgp->fileoff-=offsetr;
                            ...
                            {
                                sp = (struct symtab_command *)sgp;
                                symtab=sp;
                                symoff0=sp->symoff;
                                sp->symoff=(uint32_t)offsetl;
                                sp->stroff=(uint32_t)offsetl+sp->stroff-symoff0;
                                offset2=sp->stroff;
                                offsetlb=symoff0-offsetl;
                                offsetf=sp->stroff+sp->strsize-sp->symoff;
                                
                            }
                            if(sgp->cmd == LC_DYSYMTAB)
                            {
                                sp3 = (struct dysymtab_command *)sgp;
                                dynSymbolTable=sp3;
                               // sp3->indirectsymoff=(uint32_t)offset2;
                                
                                if (sp3->modtaboff) sp3->modtaboff=sp3->modtaboff-(uint32_t)offsetlb;
                                if (sp3->extrefsymoff) sp3->extrefsymoff=sp3->extrefsymoff-(uint32_t)offsetlb;
                                if (sp3->indirectsymoff) sp3->indirectsymoff=sp3->indirectsymoff-(uint32_t)offsetlb;
                                if (sp3->extreloff) sp3->extreloff=sp3->extreloff-(uint32_t)offsetlb;
                                if (sp3->locreloff) sp3->locreloff=sp3->locreloff-(uint32_t)offsetlb;
                            }
                            if(sgp->cmd == LC_DYLD_CHAINED_FIXUPS)
                            {
                                sp2 = (struct linkedit_data_command *)sgp;
                                offsetf=sp2->dataoff-(uint32_t)symoff0+sp2->datasize;
                                sp2->dataoff=(uint32_t)offsetl+(uint32_t)offsetf-sp2->datasize;
                            }
                        ...
                        uint8_t* linkEditBase = unslidLinkEditBase+ slide;
                        struct nlist_64* symbolTable = (struct nlist_64*)(&linkEditBase[symoff0]);

                        struct nlist_64* globalsStart= &symbolTable[dynSymbolTable->iextdefsym];
                        struct nlist_64* globalsEnd = &globalsStart[dynSymbolTable->nextdefsym];
                        for (struct nlist_64* s = globalsStart; s < globalsEnd; ++s) {
                             if ( (s->n_type & N_TYPE) == N_SECT ) {
                                 if (base) s->n_value &= ~0xffffff8000000000;
                                if (s->n_value>=offsetrm) s->n_value-=offsetrm;
                            }
                        }
                        struct nlist_64* const localsStart = &symbolTable[dynSymbolTable->ilocalsym];
                        struct nlist_64* const localsEnd= &localsStart[dynSymbolTable->nlocalsym];
                        for (struct nlist_64* s = localsStart; s < localsEnd; ++s) {
                        ...
                        /*struct nlist_64* globalsStart2= &symbolTable[dynSymbolTable->iundefsym];
                        struct nlist_64* globalsEnd2 = &globalsStart2[dynSymbolTable->nundefsym];
                        for (struct nlist_64* s = globalsStart2; s < globalsEnd2; ++s) {
                             if ( (s->n_type & N_TYPE) == N_SECT ) {
                                 if (base) s->n_value &= ~0xffffff8000000000;
                                if (s->n_value>=offsetrm) s->n_value-=offsetrm;
                            }
                        ...
                        if (codeSignature)
                        {
                            ...
                            signedKexts++;
                            //sourceSize=codeSignature->dataoff+codeSignature->datasize;
                        }
                        ...
                        if ((offset) && (sourceSize))
                        {
                            ...
                            printf("Executable....................: %s (%lx bytes written)\n", kextIdentifierBuffer, ftell(executable));
                            /*if (offsetl-offset1) {
                                unsigned char *nu=malloc(offsetl-offset1);
                                bzero(nu,offsetl-offset1);
                                fwrite(nu, 1, offsetl-offset1, executable);
                            }*/
                            sourceSize=offsetf;
                            fwrite((aFileBuffer + symoff0), 1, (size_t)sourceSize, executable);
...
#else
    f = fopen(argv[1], "rb");
   // f = fopen("/System/Library/KernelCollections/SystemKernelExtensions.kc", "rb");
   // f = fopen("/System/Library/KernelCollections/BootKernelExtensions.kc", "rb");
   // do_kext = 1;
#endif
